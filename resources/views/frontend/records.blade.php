    ...
    >
        ...
        <div class="mb-8 flex flex-wrap items-center gap-x-6 gap-y-5 sm:flex-nowrap">
            <div class="grid grow auto-cols-fr grid-flow-col gap-2 sm:gap-4">
                <a
                    href="{{ route('profile.records', ['type' => 'all']) }}"
                    class="@if(@$_GET['type'] !== 'canceled') active @endif flex min-h-11 grow items-center justify-center rounded bg-white py-2 px-4 text-center transition hover:bg-primary-50 [&amp;.active]:bg-primary-500 [&amp;.active]:text-white"
                >Все записи</a
                ><a
                    href="{{ route('profile.records', ['type' => 'canceled']) }}"
                    class="@if(@$_GET['type'] === 'canceled') active @endif flex min-h-11 grow items-center justify-center rounded bg-white py-2 px-4 text-center transition hover:bg-primary-50 [&amp;.active]:bg-primary-500 [&amp;.active]:text-white"
            ...
            <details class="dropdown group relative z-[3000] open:drop-shadow ml-auto">
                ...
                <div class="absolute top-full min-w-full rounded-b bg-white pb-2">
                    <a
                        href="{{ route('profile.records', ['date' => 'latest', 'type' => isset($_GET['type']) && $_GET['type'] === 'canceled' ? 'canceled' : 'all']) }}"
                    ...
                    ><a
                        href="{{ route('profile.records', ['date' => 'oldest', 'type' => isset($_GET['type']) && $_GET['type'] === 'canceled' ? 'canceled' : 'all']) }}"
                        class="block py-1.5 px-5 text-xs transition-[color] hover:text-primary-400 [&.active]:font-extrabold @if(@$_GET['date'] === 'oldest') active @endif"
        ...
        {{ $appointments->appends(['date' => isset($_GET['date']) ? $_GET['date'] : 'latest', 'type' => isset($_GET['type']) && $_GET['type'] === 'canceled' ? 'canceled' : 'all'])->links() }}
